...
下面是对公开开源仓库 DeepSeek Harness 生成的真实效果，覆盖 **7807 个 tracked files、1253 个目录、226 个插件包**：

[![代码库理解地图真实生成效果](assets/atlas-demo.png)](assets/atlas-demo.png)

> 点击图片可以查看大图。实际产物是完整交互式 HTML：可以搜索文件和功能、进入目录、查看模块说明，并从核心能力视角检查每个文件的角色。截图用于展示真实分析内容；当前统一页面结构以仓库内置模板和视觉规范为准。
...
- 为模块、目录和文件生成小白也能看懂的简洁中文说明，禁止直接粘贴英文 README 或源码注释；
- 用“代码名（通俗含义）”解释技术标识；
- 说明跨模块依赖提供了什么能力、当前文件为什么需要它；
- 展示路由、存储结构、主要声明、测试和有价值的源码注释；
- 为每个文件生成核心能力角色卡片：直接参与、公共支撑、自动化验证、文档说明或不参与；
- 使用仓库内置的统一页面模板，强制保持截图所示的 GitHub 风格导航栏、仓库标题、双栏目录、紧凑文件行与右侧详情抽屉；
- 生成单文件、自包含、无需服务器的交互式 HTML；
- 文件详情固定展示“主要作用、主要函数/类型/变量、直接涉及的数据表、源码设计说明”等栏目，没有证据也明确说明；
- 使用审计脚本同时检查文件覆盖、中文可读性、结构化作用、声明类型、数据表解释、角色卡片和页面骨架，避免不同 Agent 各画各的。
...
- 是否有重复或遗漏文件；
- 文件说明是否以中文为主，而不是“两个汉字加一段英文”；
- 是否具备“主要作用、使用时机、输入与结果”的结构化解释；
- 主要函数、类型和变量是否标明种类并用中文解释；
- 直接涉及的数据表是否说明中文含义、读写方式和用途；
- 每个文件是否具备完整的核心能力角色卡片；
- 是否出现空描述、空泛措辞或缺失的详情栏目；
- HTML 是否包含离线运行所需的内嵌数据。

Skill 自带行为测试：

```bash
python3 scripts/test_atlas_tools.py
```
